...
export const sendMessage = async (req: Request, res: Response) => {
  try {
    ...
    if (socket) {
      emitSocketEvent(req, messdataresult, socket?.socketId);
    }
    const userEmailData = await fetchUserEmailById(req.body.receiverId);
    if (userEmailData.length > 0) {
      const notificationCountData = await getNotification(
        req.body.senderId,
        userEmailData[0].email,
      );
      if (notificationCountData) {
        await updateNotification(
          {
            email: userEmailData[0].email,
            notificationCount: notificationCountData?.notificationCount + 1,
            userId: req.body.senderId,
          },
          notificationCountData.id,
        );
      } else {
